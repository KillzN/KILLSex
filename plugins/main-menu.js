...
let handler = async (m, { conn, usedPrefix, usedPrefix: _p, __dirname, text, command }) => {
    try {
        ...
        let perfil = await conn.profilePictureUrl(who, 'image').catch(_ => 'https://ibb.co/Y73rRC0X')
        let menu = `
...
`.trim()
        ...
        await conn.sendMessage(m.chat, {
            image: { url: 'https://ibb.co/Y73rRC0X' }, // Imagen fija reemplazando al video
            caption: menu,
            contextInfo: {
